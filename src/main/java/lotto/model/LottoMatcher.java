...
public class LottoMatcher {
    ...
    private final HashMap<Integer, Integer> scoreBoard;

    public LottoMatcher() {
        scoreBoard = new HashMap<>();
        for (int prize = 3; prize <= 7; prize++) scoreBoard.put(prize, 0);
    }
    ...
    private float calculateIncomeRate(Lottoes lottoes) {
        ...
        income += scoreBoard.get(7) * 2_000_000_000;
        return income / originMoney * 100;
    }

    private void increase(int prize) {
        scoreBoard.put(prize, scoreBoard.get(prize) + 1);
    }
